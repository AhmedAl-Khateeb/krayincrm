...
use Webkul\Admin\Notifications\LeadNoteAddedNotification;
...
class ActivityController extends Controller
{
    ...
    public function __construct(
        ...
        protected LeadRepository $leadRepository, // ✅ NEW
    ) {}
    ...
    public function store(): RedirectResponse|JsonResponse
    {
        ...
        /**
         * ✅ NEW: اربط الـ Activity بالـ Lead في store() (زي update())
         * علشان الـ note يبقى مربوط فعلاً بالـ lead.
         */
        if (request()->filled('lead_id')) {
            $activity->leads()->sync([request('lead_id')]);
        }

        /**
         * ✅ NEW: ابعت Notification لما يتضاف Note على Lead
         */
        if (request('type') === 'note' && request()->filled('lead_id')) {
            $lead = $this->leadRepository->find(request('lead_id'));

            // غالباً lead عنده user relation (assigned sales/owner)
            // لو اسم العلاقة عندك مختلف عدّل السطر ده
            $targetUser = $lead?->user;

            if ($targetUser) {
                $targetUser->notify(new LeadNoteAddedNotification(
                    lead: $lead,
                    activity: $activity,
                    createdBy: auth()->guard('user')->user()
                ));
            }
        }
