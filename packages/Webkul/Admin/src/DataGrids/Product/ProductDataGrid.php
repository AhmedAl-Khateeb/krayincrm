    ...
    public function prepareQueryBuilder(): Builder
    {
        ...
        $queryBuilder = DB::table('products')
            ...
            ->leftJoin('tags', 'tags.id', '=', 'product_tags.tag_id')
               ...
               ->leftJoin('attribute_values as plan_av', function ($join) use ($planAttributeId) {
                   $join->on('plan_av.entity_id', '=', 'products.id')
                       ->where('plan_av.entity_type', '=', 'products')
                       ->where('plan_av.attribute_id', '=', $planAttributeId);
               })

        // ✅ join attribute_options عشان اسم الاختيار
        ->leftJoin('attribute_options as plan_opt', function ($join) {
            $join->on('plan_opt.id', '=', 'plan_av.integer_value');
        })
            ...
            ->groupBy('products.id');
        $this->addFilter('plan', DB::raw('MAX(plan_opt.name)'));
